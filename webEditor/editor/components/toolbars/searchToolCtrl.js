    ...
    function ($scope, dsEdit) {
        ...
        $(document).click(function (e) {
            ...
            if (clickTarget != $('.searchInput')[0]) {
                ...
                var str = '<a href="" ng-click="selectPage(page.number, $event)" class="ng-binding">' + $scope.pagation.pageNum + '</a>';
                var isChild = $(clickTarget).parents().hasClass('searchPanel');
                if (!isChild && clickTarget.outerHTML === str) { // 解决bug10990，在google浏览器的某些版本中，由于$(clickTarget).parents()获取不到父节点，导致点击最后一页时面板会关掉。
                    isChild = true;
                }
            ...
            dsEdit.normalSearch($scope.searchParma).then(function (data) {
                if (data) {
                    ...
                    for (var i = 0; i < data.rows.length; i++) {
                        ...
                        data.rows[i].shortName = (data.rows[i].name.length > 10) ? data.rows[i].name.substring(0, 10) + '...' : data.rows[i].name;
                    }
                    $scope.searchDataList = data.rows;
                    $scope.pagation.total = data.total;
                }
            });
        };
