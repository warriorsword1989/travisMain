/**
 * 历史版本查询页面的主controller
 * @file       editorHisCtrl.js
 * @author wuzhen
 * @date   2018-01-05
 *
...
angular.module('app').controller('editorHisCtrl', ['$scope', '$rootScope', '$cookies', '$timeout', '$q',
    '$ocLazyLoad', 'ngDialog',
    ...
    function ($scope, $rootScope, $cookies, $timeout, $q, $ocLazyLoad, ngDialog,
        ...
        if (!window.opener) {
            swal({
                title: '当前页面只能通过window.open方式打开',
                showConfirmButton: false
            });
            return;
        }
        var dbId = App.Util.getUrlParam('hisDbId');
        dbId = parseInt(dbId, 0);
        App.Temp = FM.Util.clone(window.opener.getWindowAppTemp()); // 打开新窗体时App.Temp里的变量会丢失，所以需要从父窗体重获取
        App.Temp.dbId = dbId;
        ...
        $scope.closeDialog = function (dlgKey) {
            $scope.$broadcast('DoSaveOfExQuality', dlgKey);
        ...
        var showInTipLeftViewPanel = function (data) {
            var ctrl = appPath.editorHis + 'left-panels/tip/tipLeftViewPanelCtrl.js';
            var tmpl = appPath.editorHis + 'left-panels/tip/tipLeftViewPanelTmpl.htm';
        ...
        var showInPoiLeftViewPanel = function (data) {
            var ctrl = appPath.editorHis + 'left-panels/poi/poiLeftViewPanelCtrl.js';
            var tmpl = appPath.editorHis + 'left-panels/poi/poiLeftViewPanelTmpl.htm';
            dsLazyload.loadInclude($scope, 'leftPanelTmpl', ctrl, tmpl).then(function () {
        ...
        var showInRoadRightAddPanel = function (data) {
            var ctrl = appPath.editorHis + 'right-panels/road/roadRightAddPanelCtrl.js';
            var tmpl = appPath.editorHis + 'right-panels/road/roadRightAddPanelTmpl.htm';
        ...
        var showInRoadRightEditPanel = function (data) {
            var ctrl = appPath.editorHis + 'right-panels/road/roadRightEditPanelCtrl.js';
            var tmpl = appPath.editorHis + 'right-panels/road/roadRightEditPanelTmpl.htm';
            dsLazyload.loadInclude($scope, 'rightPanelTmpl', ctrl, tmpl).then(function () {
        ...
        var showInIndexRightEditPanel = function (data) {
            var ctrl = appPath.editorHis + 'right-panels/index/indexRightEditPanelCtrl.js';
            var tmpl = appPath.editorHis + 'right-panels/index/indexRightEditPanelTmpl.html';
            dsLazyload.loadInclude($scope, 'rightPanelTmpl', ctrl, tmpl).then(function () {
        ...
        var showInPoiRightEditPanel = function (data) {
            var ctrl = appPath.editorHis + 'right-panels/poi/poiRightEditPanelCtrl.js';
            var tmpl = appPath.editorHis + 'right-panels/poi/poiRightEditPanelTmpl.htm';
            dsLazyload.loadInclude($scope, 'rightPanelTmpl', ctrl, tmpl).then(function () {
        ...
        var showInTipRightEditPanel = function (data) {
            var ctrl = appPath.editorHis + 'right-panels/tip/tipRightEditPanelCtrl.js';
            var tmpl = appPath.editorHis + 'right-panels/tip/tipRightEditPanelTmpl.htm';
            dsLazyload.loadInclude($scope, 'rightPanelTmpl', ctrl, tmpl).then(function () {
        ...
        var showInBatchRightPanel = function (data) {
            var ctrl = appPath.editorHis + 'right-panels/batch/batchRightEditPanelCtrl.js';
            var tmpl = appPath.editorHis + 'right-panels/batch/batchRightEditPanelTmpl.htm';
            dsLazyload.loadInclude($scope, 'rightPanelTmpl', ctrl, tmpl).then(function () {
        ...
        var showInLeftFloatPanel = function (data) {
            var ctrl = appPath.editorHis + 'left-panels/float/leftFloatPanelCtrl.js';
            var tmpl = appPath.editorHis + 'left-panels/float/leftFloatPanelTmpl.htm';
        ...
        var showInRightFloatPanel = function (data) {
            var ctrl = appPath.editorHis + 'right-panels/float/rightFloatPanelCtrl.js';
            var tmpl = appPath.editorHis + 'right-panels/float/rightFloatPanelTmpl.htm';
        ...
        var showInDataList = function (data) {
            var ctrl = appPath.editorHis + 'dialog-panels/selectedDataListCtrl.js';
            var tmpl = appPath.editorHis + 'dialog-panels/selectedDataListTmpl.html';
        ...
        var _objectLoadedCallback = function (geoLiveType, data, selectEvent) {
            ...
            $rootScope.Editable = false; // 控制所有属性都不能编辑
        ...
        var loadObject = function (data) {
            ...
            // add by wuzhen on 2018-01-17 渲染接口返回备份数据的dbid比较复杂,前端直接赋值为备份库的dbid（暂时不涉及接边，如果涉及接边时就需要服务端进行修改）
            data.feature.dbId = App.Temp.dbId;
        ...
        var loadComponents = function () {
            // 右侧工具条-用户
            $scope.searchToolTmpl = appPath.editorHis + 'toolbars/searchToolTmpl.htm';
            // 右侧工具条-地图工具
            $scope.rightMapToolTmpl = appPath.editorHis + 'toolbars/mapToolPanelTmpl.htm';

            // $ocLazyLoad.load(appPath.editorHis + 'toolbars/editToolCtrl.js').then(function () {
            //     $scope.editToolTmpl = appPath.editorHis + 'toolbars/editToolTmpl.htm';
            // });

            // 专题图图例
            $ocLazyLoad.load(appPath.editorHis + 'toolbars/thematicFigureCtrl.js').then(function () {
                $scope.thematicFigureTmpl = appPath.editorHis + 'toolbars/thematicFigureTmpl.html';
            });
        };
        ...
        var initialize = function () {
            ...
            // 解决切换任务后，地图不能正常加载的问题
            $timeout(function () {
                $scope.$broadcast('Map-Initialize', { dbId: App.Temp.dbId });
            }, 100);
        ...
        // 接收父窗体通过window.postMessage发送过来的数据
