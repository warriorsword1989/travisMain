...
angular.module('app').controller('mainMapHisCtrl', ['$scope', '$cookies', '$q', '$timeout', '$ocLazyLoad', 'hotkeys',
    'dsLazyload', 'appPath',
    function ($scope, $cookies, $q, $timeout, $ocLazyLoad, hotkeys, dsLazyload, appPath) {
        ...
        var layerCtrl;
        var syncFlag = false; // 需求是任意一边的地图移动都会引起另一边的地图的移动，用此变量控制两边地图相互循环移动的问题

        $scope.selectTool = {
            name: 'PAN', // 默认是漫游工具 ，此变量用于控制正在使用的功能的背景加深显示，
            syncTool: ['LINKAGE', 'ZOOM'] // 用于历史版本查看页面的选中工具背景加深显示的控制
        };

        // 加载各种配置文件
        var loadConfigs = function (dbId) {
            ...
            App.Config.map.SourceConfig.objSource.requestParameter.dbId = dbId;
            App.Config.map.SourceConfig.thematicSource.requestParameter.dbId = dbId;
        ...
        var loadMapToolbar = function () {
            var promises = [];
            promises.push($ocLazyLoad.load(appPath.editorHis + 'map/mapToolbarCtrl.js'));
            $q.all(promises).then(function () {
                $scope.mapToolbarTmpl = appPath.editorHis + '/map/mapToolbarTmpl.htm';
            });
        };
        ...
        //     var ctrl = appPath.editorHis + 'map/mapToolbarPanelCtrl.js';
        //     var tmpl = appPath.editorHis + 'map/mapToolbarPanelTmpl.html';
        ...
        /**
         * 同步父窗体的位置
         * @param {Object} e 地图移动结束后回调的参数
         * @returns {null} null
         */
        var linkageTootherMap = function (e) {
            var tools = $scope.selectTool.syncTool;
            if (tools && tools.length > 0) {
                var data = {
                    center: '',
                    zoom: 0
                };
                if (tools.indexOf('LINKAGE') > -1) {
                    var center = e.target.getCenter();
                    data.center = center.lat + ',' + center.lng;
                }
                if (tools.indexOf('ZOOM') > -1) {
                    data.zoom = e.target.getZoom();
                }
                var url = 'http://' + window.location.host + '/apps/webEditor/editor';
                if (window.opener) {
        ...
        $scope.$on('Map-LocationOrZoom', function (event, data) {
            ...
            if ($scope.selectTool.syncTool.indexOf('LINKAGE') > -1) {
                center = L.latLng(data.center.split(','));
            }
            if ($scope.selectTool.syncTool.indexOf('ZOOM') > -1) {
                zoom = data.zoom;
            }
            $scope.map.setView(center, zoom);
        });
