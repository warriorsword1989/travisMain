/**
 * 多源任务列表
 * @file taskMutiSourceTableCtrl.js
 * @author wangmingdong
 * @date   2017-11-08
 *
...
angular.module('app').controller('taskMutiSourceTableCtrl', ['$scope', 'ngDialog', 'dsManage', 'dsFcc', '$timeout', 'dsColumn', 'appPath', 'dsEdit',
    function ($scope, ngDialog, dsManage, dsFcc, $timeout, dsColumn, appPath, dsEdit) {
        ...
        var getChildTaskList = function (data) {
            ...
            $timeout(function () {
                ...
                //         if ($scope.taskListData[i].subtaskId === App.Temp.subTaskId) {
                //             $scope.taskListData[i].checked = true;
                //         }
                //     }
                // }
            });
         ...
         */
        function getIndex() {
            return '<div class="ui-grid-cell-contents"><input type="checkbox" ng-model="row.entity.checked" ng-click="grid.appScope.checkTaskRow(row, 1, $event);" class="fm-control"/><span style="vertical-align: middle">{{grid.appScope.caculateIndex(grid,row)}}</span></div>';
        }
        ...
        function getOperation() {
            var html = '<div class="ui-grid-cell-contents"><div class="task-underline-red" ng-click="grid.appScope.submitTask($event, row.entity)">关闭</div>';
            return html;
        }
        ...
        $scope.checkTaskRow = function (row, type, event) {
            var i;
            var task;
            let count = $scope.currentPage * $scope.currentPageSize;
            let currentPageData = $scope.taskListData.slice(($scope.currentPage - 1) * $scope.currentPageSize, count);
            if (type) {
                task = row.entity;
                // 勾选单条
                if (type === 2) {
                ...
                for (i = 0; i < currentPageData.length; i++) {
                    if (!currentPageData[i].checked) {
                        $scope.checkboxes.checked = false;
                        break;
                    }
                    if (currentPageData[i].checked && i === currentPageData.length - 1) {
                ...
                // 全选/反选
                for (i = 0; i < currentPageData.length; i++) {
                    currentPageData[i].checked = $scope.checkboxes.checked;
                }
                // 反选行选中状态
        ...
        function initTaskTable() {
            $scope.gridOptions = {
                ...
                columnDefs: [{ field: 'pageIndex',
                    ...
                    enableSorting: true,
                    visible: true,
                    cellTemplate: '<div class="ui-grid-cell-contents"><span class="task-underline-blue" ng-click="grid.appScope.selectSubTask(row.entity);">{{row.entity.name}}</span></div>'
                }, {
                    field: 'subtaskId',
                    displayName: '子任务ID',
                    minWidth: 80,
                    enableSorting: true,
                    visible: true,
                    cellTemplate: '<div class="ui-grid-cell-contents"><span class="task-underline-blue" ng-click="grid.appScope.selectSubTask(row.entity);">{{row.entity.subtaskId}}</span></div>'
                }, {
                    field: 'isQuality',
                    displayName: '环节',
                    enableSorting: true,
