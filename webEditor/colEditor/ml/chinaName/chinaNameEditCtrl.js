...
angular.module('app').controller('chinaNameEditCtrl', ['$scope', '$sce', 'dsColumn', 'ngDialog', 'dsLazyload', '$timeout', 'appPath', 'hotkeys',
    function ($scope, $sce, dsColumn, ngDialog, dsLazyload, $timeout, appPath, hotkeys) {
       ...
       */
        function get12Name() {
            return '<div>{{row.entity.name12Chi.name}}</div>';
       ...
       */
        function get11NameInput() {
            var html = "<div><input type='text' maxlength='64' value='{{row.entity.name11Chi.name}}' ng-focus='grid.appScope.autoCopyValue(1, row,\"name11Chi\")' ng-blur='grid.appScope.autoCopyValue(0)' ng-model='row.entity.name11Chi.name' /></div>";
            return html;
            // var html = "<input type='text' maxlength='200' title='{{row.name11Chi.name}}' value='row.name11Chi.name' ng-model='row.name11Chi.name' />";
            // return html;
        }
      /**
       * get11Name
       ...
       */
        ...
        $scope.getRefMsg = function (row) {
            ...
            for (var i = 0; i < refMsgs.length; i++) {
                html += (i + 1) + '."' + refMsgs[i].key + '"应为"' + refMsgs[i].result + '";';
                html += '<br>';
            }
       ...
       */
        function getDetails() {
            // return '<span class="badge photoButton" data-animation="am-flip-x" data-auto-close="0" bs-popover data-placement="left"' +
            //     'data-template-url="./colEditor/chinaName/nameDetailTpl.html" >查看</span>';
       ...
       */
        function formatShortName() {
            var html = '<div style="display: flex;">' +
                '<div><div ng-repeat="item in row.entity.name51ChiArr">' +
        ...
        $scope.deleteShortName = function (row, index) {
            row.name51ChiArr.splice(index, 1);
        };
        /**
         * 过去groupNameId
        ...
        $scope.addShortName = function (row, e) { // 中文简称
            ...
            var shortName = new FM.dataApi.ColPoiName({ nameClass: 5, nameType: 1, langCode: 'CHI', poiPid: row.pid, nameGroupid: groupId });
            row.namesTemp.push(shortName);
            row.name51ChiArr.unshift(shortName);
        };
        // 别名编辑框
      /**
       * formatAliasName
      ...
      */
        function formatAliasName() {
            var html = '<div style="display: flex;">' +
                '<div><div ng-repeat="item in row.entity.name31ChiArr">' +
        ...
        $scope.addAliasName = function (row, e) {
            var groupId = getNameGroupId(row); // 获取namegroupId
            var shortName = new FM.dataApi.ColPoiName({ nameClass: 3, nameType: 1, langCode: 'CHI', poiPid: row.pid, nameGroupid: groupId });
            row.namesTemp.push(shortName);
            row.name31ChiArr.unshift(shortName);
            console.log($scope.currentEdited);
        };
        $scope.deleteAliasName = function (row, index) {
            row.name31ChiArr.splice(index, 1);
        };
        // 曾用名的编辑框
      /**
       * combinOldName
       ...
       * @return {String} <div><div/>
       */
        function combinOldName(scope, row) {
            var hg = 35;
            var i = 0;
            var len = 0;
            var shortNameLen = row.name61ChiArr.length;
            if (shortNameLen > 1) {
                hg = shortNameLen * 36;
            }
            var html = '';
            html += '<div style="float: right;"><button type="button" ng-click="addOldName(row, $event);" style="padding: 6px 5px;height: ' + hg + 'px;" class="btn btn-info"><span class="glyphicon glyphicon-plus"></span></button></div>';
            for (i = 0, len = row.name61ChiArr.length; i < len; i++) {
                var ht = '<div class="input-group" style="margin-right: 30px;margin-bottom: 3px">'
                    + '<input type="' + this.inputType + '" maxlength="200" class="form-control" title="{{row.name61ChiArr[' + i + '].name}}" ng-model="row.name61ChiArr[' + i + '].name">'
                    + '<span class="input-group-addon" style="padding:6px 6px;cursor: pointer;"><a ng-click="deleteOldName(row,' + i + ')"><span class="glyphicon glyphicon-remove"></span></a></span></div>';
                html += ht;
            }
            return '<div>' + html + '</div>';
        }
        $scope.addOldName = function (row, e) {
            ...
            row.namesTemp.push(shortName);
            row.name61ChiArr.unshift(shortName);
        };
        $scope.deleteOldName = function (row, index) {
            row.name61ChiArr.splice(index, 1);
        };
        ...
        // $ocLazyLoad.load('./colEditor/common/showImageCtrl.js');
       ...
       */
        ...
        var chinaNameCols = {
            ...
            nameUnify: [
                ...
                { field: 'name12Chi.name', displayName: '官方原始名称', enableSorting: true, cellTemplate: get12Name, visible: true, minWidth: 220 },
                { field: 'parentName', displayName: '父名称', enableSorting: true, visible: true, minWidth: 180 },
                { field: 'name11Chi.name', displayName: '官方标准中文名称', enableSorting: true, cellTemplate: get11NameInput, visible: true, minWidth: 200 }, // html: true 表示会显示成input框
                { field: 'refMsg', displayName: '参考信息', cellTemplate: formatRefMsg, visible: true, minWidth: 200 }
            ],
            shortName: [
                { field: 'name11Chi.name', displayName: '官方标准中文名称', enableSorting: true, cellTemplate: get11Name, visible: true, minWidth: 240 },
                { field: 'name51ChiArr', displayName: '简称', cellTemplate: formatShortName, visible: true, minWidth: 200 },
                { field: 'refMsg', displayName: '参考信息', cellTemplate: formatRefMsg, visible: true, minWidth: 240 }
            ],
            aliasName: [
                { field: 'name11Chi.name', displayName: '官方标准中文名称', enableSorting: true, cellTemplate: get11Name, visible: true, minWidth: 250 },
                { field: 'name31ChiArr', displayName: '别名', cellTemplate: formatAliasName, visible: true, minWidth: 240 },
                { field: 'refMsg', displayName: '参考信息', cellTemplate: formatRefMsg, visible: true, minWidth: 240 }
            ],
            namePinyin: [
       ...
       */
        ...
        var nameIsEmpty = function (dataList) {
            ...
            if ($scope.main.workType === 'nameUnify') {
                ...
                for (i = 0, len = dataList.length; i < len; i++) {
                    nameObj = dataList[i].name51ChiArr;
                ...
                for (i = 0, len = dataList.length; i < len; i++) {
                    nameObj = dataList[i].name31ChiArr;
                    for (j = 0; j < nameObj.length; j++) {
                        if (nameObj[j].name === '') {
                            flag = true;
                            break;
        ...
        $scope.doReplace = function () {
            ...
            if ($scope.searchObject.searchType === 'standardname') {
                var name = currentData.name11Chi.name; // 官方标准
                ...
                currentData.name11Chi.name = finalyValue;
            }
            setFocusByIndex();
        };
        /**
        ...
        $scope.doReplaceAll = function () {
            ...
            for (var i = 0; i < $scope.currentEdited.length; i++) {
                ...
                if ($scope.searchObject.searchType === 'standardname') {
                    var name = temp.name11Chi.name; // 官方标准
                    if (searchText) {
                        if ((name && name.indexOf(searchTextDbc) > -1) || (name && name.indexOf(searchTextCbd) > -1)) {
                            ...
                            temp.name11Chi.name = finalyValue;
                        }
                    } else {
                        if (!name) {
                            count += 1;
                            temp.name11Chi.name = replaceVal;
                        }
                    }
                }
            }
