...
angular.module('app').controller('engNameCtrl', ['$scope', '$sce', '$ocLazyLoad', 'ngDialog', 'dsColumn', 'dsEdit', '$interval', '$timeout', 'uiGridConstants', 'appPath',
    function ($scope, $sce, $ocLazyLoad, ngDialog, dsColumn, dsEdit, $interval, $timeout, uiGridConstants, appPath) {
        ...
        var filterKey = { // 搜索内容变化时使用
            photoEngName: 'name11Chi',
            chiEngName: 'name11Chi',
            confirmEngName: 'name11Chi',
            officalStandardEngName: 'name11Chi',
            nonImportantLongEngName: 'name11Chi'
        };
       ...
       */
        function get11EngName() {
            return '<div ng-if="row.entity.name11Chi && row.entity.name11Chi.name">{{row.entity.name11Eng.name}}</div>';
        }
      /**
       * get11ChiName
       * @return {String} <div><div/>
       */
        function get11ChiName() {
            return '<div ng-if="row.entity.name11Chi && row.entity.name11Chi.name">{{row.entity.name11Chi.name}}</div>';
        }
      /**
       * formatCkRules
       * @return {String} <div><div/>
      ...
      /**
       * get31EngName
       * @param {Object} scope 要素获取
       * @param {Object} row 要素获取
       * @return {Array} namestrArr.join
       */
        function get31EngName(scope, row) {
            var namestrArr = [];
            if (row.name31EngArr.length > 0) {
                for (var i = 0; i < row.name31EngArr.length; i++) {
                    namestrArr.push(row.name31EngArr[i].name);
                }
            }
       ...
       */
        ...
        var conbineCols = function () {
            ...
            var engNameCols = {
                ...
                // 照片录入英文名
                photoEngName: [
                    { field: 'name11Chi.name', displayName: '标准中文名称', enableSorting: true, cellTemplate: get11ChiName, visible: true },
                ...
                officalStandardEngName: [
                    { field: 'name11Chi.name', displayName: '标准中文名称', enableSorting: true, cellTemplate: get11ChiName, visible: true },
                    { field: 'name11Eng.name', displayName: '官方标准英文名称', enableSorting: true, cellTemplate: get11EngName, visible: true },
                    { field: 'ckRules', displayName: '错误', cellTemplate: formatCkRules, visible: alreadyChecked, minWidth: 200 }
                ],
                // 非重要分类英文超长
       ...
       */
        ...
        $ocLazyLoad.load('./colEditor/ml/engName/engNameEditCtrl.js');
        ...
        var openEditDialog = function (isCollectData) {
            ...
            var dialog = ngDialog.open({
                template: 'colEditor/ml/engName/engNameEditTpl.html',
                controller: 'engNameEditCtrl',
        ...
        var setSearchPlaceholder = function () {
            var index = ['photoEngName', 'chiEngName', 'confirmEngName', 'officalStandardEngName', 'nonImportantLongEngName'].indexOf($scope.main.workType);
            if (index > -1) {
                $scope.searchPlaceholder = '标准中文名称';
            } else {
        ...
        // ----------- 批量 -----------
        var otherWorkItemReplace = [{ id: 'originalengname', label: '原始英文名称' }];
        var otherWorkItemSearch = [{ id: 'originalengname', label: '原始英文名称' }, { id: 'standardname', label: '官方标准中文名称' }];
        var offiStandEngNameReplace = [{ id: 'standardengname', label: '官方标准英文名' }];
        var offiStandEngNameSearch = [{ id: 'standardengname', label: '官方标准英文名' }, { id: 'standardname', label: '官方标准中文名称' }];

        $scope.replaceList = otherWorkItemReplace;
        $scope.searchList = otherWorkItemSearch;
        ...
        $scope.collectData = function (type) {
            ...
            for (var i = 0; i < $scope.allTableList.length; i++) {
                ...
                if ($scope.searchObject.searchType === 'standardname') {
                    ...
                    name = temp.name11Eng.name; // 官方标准英文
        ...
        $scope.$on('refreshChildrenPage', function (event, obj) {
            if ($scope.main.workType === 'officalStandardEngName') { // 官方标准英文名
                $scope.replaceList = offiStandEngNameReplace;
                $scope.searchList = offiStandEngNameSearch;
                $scope.enableBatch = true;
            } else if ($scope.main.workType === 'aliasOriEngName' || $scope.main.workType === 'aliasStdEngName') {
