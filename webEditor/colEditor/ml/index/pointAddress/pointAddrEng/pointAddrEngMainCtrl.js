...
angular.module('app').controller('pointAddrEngMainCtrl', ['$scope', 'ngDialog', '$ocLazyLoad',
    ...
    function ($scope, ngDialog, $ocLazyLoad, appPath, dsColumn, dsEdit, $interval, $cookies, $timeout) {
        ...
        $ocLazyLoad.load('./colEditor/ml/index/pointAddress/pointAddrEng/pointAddrEngCtrl.js').then(function () {
            $scope.taskListUrl = './colEditor/ml/index/pointAddress/pointAddrEng/pointAddrEngTpl.html';
        });
        ...
        $scope.applyPoi = function () {
            if ($scope.isQuality) {
                ...
                $ocLazyLoad.load('./colEditor/common/screenQuaDataCtrl.js').then(function () {
                    ...
                    $timeout(function () {
                        $scope.$broadcast('refreshScreenQua', 'pointAddr_engAddr');
                    }, 500);
                });
            } else {
        ...
        $scope.doSubmit = function () {
            $scope.showLoading.flag = true;
            dsColumn.submitPointAddressData('pointAddr_engAddr').then(function (jobId) {
                if (jobId) {
