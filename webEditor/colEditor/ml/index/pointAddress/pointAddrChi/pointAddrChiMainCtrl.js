...
angular.module('app').controller('pointAddrChiMainCtrl', ['$scope', 'ngDialog', '$ocLazyLoad',
    ...
    function ($scope, ngDialog, $ocLazyLoad, appPath, dsColumn, dsEdit, $interval, $timeout, $cookies) {
        ...
        $ocLazyLoad.load('./colEditor/ml/index/pointAddress/pointAddrChi/pointAddrChiCtrl.js').then(function () {
            $scope.taskListUrl = './colEditor/ml/index/pointAddress/pointAddrChi/pointAddrChiTpl.html';
        });
        ...
        $scope.applyPoi = function () {
            if ($scope.isQuality) {
                ...
                $ocLazyLoad.load('./colEditor/common/screenQuaDataCtrl.js').then(function () {
                    ...
                    $timeout(function () {
                        $scope.$broadcast('refreshScreenQua', 'pointAddr_chiAddr');
                    }, 500);
                });
            } else {
                ...
                dsColumn.applyPointAddress(param).then(function (data) {
                    $scope.showLoading.flag = false;
                    if (data == 0) {
        ...
        $scope.doSubmit = function () {
            $scope.showLoading.flag = true;
            dsColumn.submitData('pointAddr_chiAddr').then(function (jobId) {
                if (jobId) {
        ...
        $scope.queryStatistics = function () {
            ...
            dsColumn.queryPointSecondWorkStatistics(param).then(function (data) {
                if (data) {
                    if (data.total) {
