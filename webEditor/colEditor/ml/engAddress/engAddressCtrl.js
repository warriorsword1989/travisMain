...
angular.module('app').controller('engAddressCtrl', ['$scope', '$sce', '$ocLazyLoad', 'ngDialog', 'dsColumn', 'dsEdit', '$interval', '$timeout', 'uiGridConstants', 'appPath',
    function ($scope, $sce, $ocLazyLoad, ngDialog, dsColumn, dsEdit, $interval, $timeout, uiGridConstants, appPath) {
        ...
        var filterKey = { // 搜索内容变化时使用
            engMapAddress: 'addressChi',
            nonImportantLongEngAddress: 'addressChi'
        };

        $scope.main.workType = 'engMapAddress'; // 默认加载项
       ...
       * @return {String} <div><div/>
       */
        function getAddrChiFullname() {
            var html = '<div>{{row.entity.addressChi.fullname}}</div>';
            return html;
        }
      /**
       *formatAddressChiCombine
       * @return {String} <div><div/>
       */
        function formatAddressChiCombine() {
            var html = '<div>{{grid.appScope.getAddressChiCombine(row.entity)}}</div>';
            return html;
        }
        ...
        var conbineCols = function () {
            ...
            var engNameCols = {
                ...
                primaryCols: [
                    ...
                    { field: 'addressChi.fullname', displayName: '全地址', enableSorting: true, cellTemplate: getAddrChiFullname, visible: true, minWidth: 150 }
                ],
                // 重要分类地址英文作业
                engMapAddress: [
                    { field: 'addressChi.roadnameStr', displayName: '中文地址合并', enableSorting: true, cellTemplate: formatAddressChiCombine, visible: true, minWidth: 150 },
       ...
       */
        ...
        $ocLazyLoad.load('./colEditor/ml/engAddress/engAddressEditCtrl.js');
        ...
        var openEditDialog = function (isCollectData) {
            ...
            var dialog = ngDialog.open({
                template: 'colEditor/ml/engAddress/engAddressEditTpl.html',
