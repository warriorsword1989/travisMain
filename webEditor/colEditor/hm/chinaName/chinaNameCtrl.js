...
angular.module('app').controller('hmChinaNameCtrl', ['$scope', '$sce', '$ocLazyLoad', 'ngDialog', 'dsColumn', '$interval', 'dsEdit', '$timeout', 'uiGridConstants',
    function ($scope, $sce, $ocLazyLoad, ngDialog, dsColumn, $interval, dsEdit, $timeout, uiGridConstants) {
       ...
       */
        function get12Name() {
            var html = '<div ng-if="row.entity.name12Cht && row.entity.name12Cht.name" >{{row.entity.name12Cht.name}}</div>';
            return html;
        }
      /**
       * get11Name
       ...
       */
        function formatShortName() {
            var html = '<div ng-if="row.entity.name51ChtArr.length">{{grid.appScope.getShortName(row.entity.name51ChtArr)}}</div>';
            return html;
        }

        $scope.getShortName = function (name51ChtArr) {
            var namestrArr = [];
            if (name51ChtArr.length > 0) {
                for (var i = 0; i < name51ChtArr.length; i++) {
                    namestrArr.push(name51ChtArr[i].name);
                }
            }
       ...
       */
        function combinAliasName(scope, row) {
            var html = '<div>{{grid.appScope.getCombinAliasName(row.entity.name31ChtArr)}}</div>';
            return html;
        }

        $scope.getCombinAliasName = function (name31ChtArr) {
            var namestrArr = [];
            if (name31ChtArr.length > 0) {
                for (var i = 0; i < name31ChtArr.length; i++) {
                    if (name31ChtArr[i].name) {
                        namestrArr.push(name31ChtArr[i].name);
                    }
                }
            }
            return namestrArr.join(',');
       ...
       */
        ...
        var parseNamesMultiPinyin = function (list) {
            ...
            for (;i < list.length; i++) {
                ...
                for (j = 0; j < nameArr.length; j++) {
                    // 存在多音字，并且是大陆,港澳暂时不支持
                    if (nameArr[j].multiPinyin && nameArr[j].multiPinyin.length > 0 && nameArr[j].langCode === 'CHT') {
        ...
        var conbineCols = function () {
            var chinaNameCols = { // 配置显示的列
                ...
                // 名称统一
                nameUnify: [
                    { field: 'name12Cht.name', displayName: '官方原始名称', enableSorting: true, cellTemplate: get12Name, visible: true, minWidth: 150 },
                ...
                shortName: [
                    { field: 'name11Cht.name', displayName: '官方标准中文名称', enableSorting: true, cellTemplate: get11Name, visible: true, minWidth: 150 },
                    { field: 'name51ChtArr', displayName: '简称', cellTemplate: formatShortName, visible: true, minWidth: 150 },
                    { field: 'ckRules', displayName: '错误', cellTemplate: getCkRules, visible: alreadyChecked, minWidth: 200 }
                ],
                // 别名作业
                aliasName: [
                    { field: 'name11Cht.name', displayName: '官方标准中文名称', enableSorting: true, cellTemplate: get11Name, visible: true, minWidth: 150 },
                    { field: 'name31ChtArr', displayName: '别名', cellTemplate: combinAliasName, visible: true, html: true, inputType: 'text', minWidth: 150 },
      ...
      */
       ...
       */
        ...
        $ocLazyLoad.load('./colEditor/hm/chinaName/chinaNameEditCtrl.js');
        ...
        var openEditDialog = function (isCollectData) {
            ...
            var dialog = ngDialog.open({
                template: 'colEditor/hm/chinaName/chinaNameEditTpl.html',
                controller: 'hmChinaNameEditCtrl',
        ...
        $scope.collectData = function (type) {
            ...
            for (var i = 0; i < $scope.allTableList.length; i++) {
                ...
                if ($scope.searchObject.searchType === 'standardname') {
                    name = temp.name11Cht.name; // 官方标准
                } else if ($scope.searchObject.searchType === 'originalname') {
                    name = temp.name12Cht.name; // 官方原始
                }
                if (searchText) {
