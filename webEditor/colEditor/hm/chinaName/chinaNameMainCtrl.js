...
angular.module('app').controller('hmChinaNameMainCtrl', ['$scope', 'ngDialog', '$ocLazyLoad',
    ...
    function ($scope, ngDialog, $ocLazyLoad, appPath, dsColumn, dsEdit, $interval, $cookies, $timeout) {
        ...
        $scope.userName = '未登录'; // 详情的时候需要显示登陆名
        ...
        $ocLazyLoad.load('./colEditor/hm/chinaName/chinaNameCtrl.js').then(function () {
            $scope.taskListUrl = './colEditor/hm/chinaName/chinaNameTpl.html';
        });
        ...
        $scope.applyPoi = function () {
            if ($scope.isQuality) {
                ...
                $ocLazyLoad.load('./colEditor/common/screenQuaDataCtrl.js').then(function () {
                    ...
                    $timeout(function () {
                        $scope.$broadcast('refreshScreenQua', 'poi_name');
                    }, 500);
                });
            } else {
        ...
        $scope.doSubmit = function () {
            $scope.showLoading.flag = true;
            dsColumn.submitData('poi_name').then(function (jobId) {
                if (jobId) {
