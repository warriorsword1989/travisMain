...
angular.module('app').controller('hmChinaAddressCtrl', ['$scope', '$sce', '$ocLazyLoad', 'ngDialog', 'dsColumn', 'dsEdit', '$interval', '$timeout', 'uiGridConstants',
    function ($scope, $sce, $ocLazyLoad, ngDialog, dsColumn, dsEdit, $interval, $timeout, uiGridConstants) {
        ...
        $scope.languageFlag = 'addressCht'; // addressChi--大陆 addressCht--港澳
        var filterKey = { // 搜索内容变化时使用
            addrSplit: 'addressCht',
            addrPinyin: 'name11Cht'
        };
       ...
       */
        ...
        $scope.getClassifyRules = function (classifyRules) {
            return App.Config.Constant.poi.classifyRules[classifyRules];
        };
      /**
       * 获取名称
       * @return {String} <div><div/>
       */
        function get11Name() {
            var html = '<div ng-if="row.entity.name11Cht && row.entity.name11Cht.name" >{{row.entity.name11Cht.name}}</div>';
            return html;
        }
      /**
       * 获取全名
       ...
       */
        ...
        var conbineCols = function () {
            var chinaNameCols = { // 配置显示的列
                ...
                // 地址作业
                addrSplit: [
                    { field: 'name11Cht.name', displayName: '官方标准中文名称', enableSorting: true, cellTemplate: get11Name, visible: true, minWidth: 150 },
                    { field: 'addressCht.fullname', displayName: '地址全称', enableSorting: true, cellTemplate: getFullName, visible: true, minWidth: 150 }
                ],
                // 拼音作业
                addrPinyin: [
                    { field: 'name11Cht.name', displayName: '官方标准中文名称', enableSorting: true, cellTemplate: get11Name, visible: true, minWidth: 170 },
                    { field: 'addressCht.roadnameStr', displayName: '中文地址合并', enableSorting: true, cellTemplate: $scope.formatCombineName, visible: true, minWidth: 150 },
                    { field: 'addressCht.roadnamePhoneticStr', displayName: '拼音地址合并', cellTemplate: $scope.formatCombinePinyin, visible: true, minWidth: 150 }
                ],
                // 后公共列
                lastCols: [
       ...
       */
        ...
        $scope.singleFilter = function (renderableRows) {
            ...
            renderableRows.forEach(function (row) {
                ...
                [$scope.searchField].forEach(function (field) {
                    if ($scope.searchText && !FM.Util.isEmptyObject(row.entity[field]) && field == 'addressCht' && (row.entity[field].fullname.indexOf(searchTextDbc) > -1 || row.entity[field].fullname.indexOf(searchTextCbd) > -1)) {
                        match = true;
                    } else if ($scope.searchText && !FM.Util.isEmptyObject(row.entity[field]) && field == 'name11Cht' && (row.entity[field].name.indexOf(searchTextDbc) > -1 || row.entity[field].name.indexOf(searchTextCbd) > -1)) {
                        match = true;
        ...
        $ocLazyLoad.load('./colEditor/hm/chinaAddress/chinaAddressEditCtrl.js');
        ...
        var openEditDialog = function (isCollectData) {
            ...
            var dialog = ngDialog.open({
                template: 'colEditor/hm/chinaAddress/chinaAddressEditTpl.html',
                controller: 'hmChinaAddressEditCtrl',
        ...
        $scope.collectData = function (type) {
            ...
            for (var i = 0; i < $scope.allTableList.length; i++) {
                ...
                if ($scope.searchObject.searchType === 'standardname') {
                    name = temp.name11Cht.name; // 官方标准中文
                } else if ($scope.searchObject.searchType === 'fullname') {
                    name = temp.addressCht.fullname; // 地址全称
                } else { // 地址的18个字段
                    name = temp.addressCht[$scope.searchObject.searchType]; // 原始英文
                }
                if (searchText) {
